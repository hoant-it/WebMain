...
module.exports.OrderDraftImportExcel = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < jsonPagesArray[0].content.length; i++) {
      await db.DONHANGITEM_DRAFT_Insert_Web_V1(jsonPagesArray[0].content[i],req.signedCookies.userId)
    }
...
module.exports.OrderImportExcel = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < jsonPagesArray[0].content.length; i++) {
      await db.DONHANGITEM_3_Insert_Web_V1(jsonPagesArray[0].content[i],req.signedCookies.userId)
    }
